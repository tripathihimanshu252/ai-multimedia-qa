...
async def test_me_authorized(client):
    with patch("database.users_collection.find_one", new_callable=AsyncMock, return_value=FAKE_USER), \
         patch("routes.auth.verify_password", return_value=True), \
         patch("routes.auth.create_token", return_value="fake.jwt.token"):
        login = await client.post("/api/auth/login", json={"email":"test@example.com","password":"pass123"})
        token = login.json().get("access_token", "fake.jwt.token")
    with patch("routes.auth.decode_token", return_value={"sub": str(FAKE_USER["_id"])}), \
         patch("database.users_collection.find_one", new_callable=AsyncMock, return_value=FAKE_USER):
